# src/feature_engineering_tabpfn.py
...
def create_and_save_tabular_dataframe(engineered_features, feature_names, y_windows, subject_ids, start_times, end_times, target_col, subject_col, output_path):
    """
    Combines engineered features and metadata into a single tabular DataFrame
    and saves it as a pickle file. This format is suitable for models like TabPFN.
    """
    logging.info("Creating a single tabular DataFrame for model input...")

    if engineered_features.shape[0] != len(y_windows):
        logging.error("Mismatch in number of rows between engineered features and labels. Cannot create DataFrame.")
        raise ValueError("Feature and label row counts do not match.")

    # Create the main features DataFrame
    df = pd.DataFrame(engineered_features, columns=feature_names, dtype=np.float32)

    # Add metadata and target columns
    # Use insert to make the target column one of the first for clarity
    df.insert(0, target_col, y_windows)
    df.insert(1, subject_col, subject_ids)
    df.insert(2, 'window_start_time', start_times)
    df.insert(3, 'window_end_time', end_times)

    logging.info(f"Created DataFrame with shape: {df.shape}")
    logging.info(f"Columns: {df.columns.tolist()}")

    # Save the DataFrame
    utils.save_pickle(df, output_path)
    logging.info(f"Successfully saved tabular DataFrame for TabPFN to: {output_path}")
...
def run_feature_engineering(input_dataframe_path, config):
    ...
    The primary output is a single tabular DataFrame for use with TabPFN.
    """
    ...
    if plot_config.get('enable_window_plotting', False):
        ...
        plot_output_dir = plot_config.get('window_plots_output_dir', 'window_plots')
        base_results_dir = config.get('results_dir', '.')
        full_plot_output_dir = os.path.join(base_results_dir, plot_output_dir)
        
        plot_windows_to_pdf(
            X_windows_raw, y_windows, subject_ids_windows, window_start_times,
            actual_feature_cols, config, base_output_dir=full_plot_output_dir
        )
    else:
    ...
    if num_nan_labels > 0:
        logging.warning(f"Found {num_nan_labels} windows with NaN labels. Removing them.")
    ...
    for i in range(0, n_total_windows, FEAT_ENG_BATCH_SIZE):
        ...
        engineered_features_batch, engineered_feature_names_batch = engineer_features_for_windows(
            X_batch_raw, actual_feature_cols, sampling_rate
        )
    ...
    if not all_engineered_features_list:
        logging.error("Engineered feature calculation resulted in no features. Cannot proceed.")
    ...
    # --- Save Final Tabular DataFrame for TabPFN ---
    output_dir = config.get('intermediate_feature_dir', 'features')
    os.makedirs(output_dir, exist_ok=True)
    output_paths = {}

    try:
        logging.info("Creating and saving the final tabular DataFrame for model input.")
        tabular_df_path = os.path.join(output_dir, 'tabular_features_for_tabpfn.pkl')
        
        create_and_save_tabular_dataframe(
            engineered_features=engineered_features,
            feature_names=final_engineered_feature_names,
            y_windows=y_windows,
            subject_ids=subject_ids_windows,
            start_times=window_start_times,
            end_times=window_end_times,
            target_col=target_col,
            subject_col=subject_col,
            output_path=tabular_df_path
        )
        output_paths['tabular_dataframe_for_tabpfn'] = tabular_df_path

        # Optionally save intermediate arrays if needed for debugging
        save_intermediates = config.get('save_intermediate_arrays', False)
        if save_intermediates:
            logging.info("Saving intermediate arrays for debugging...")
            ...
            logging.info("Intermediate arrays saved.")

    except Exception as e:
        logging.error(f"Error saving feature engineering outputs: {e}", exc_info=True)
        raise

    logging.info(f"Feature engineering stage complete. Primary output at: {output_paths['tabular_dataframe_for_tabpfn']}")
    return output_paths
...
if __name__ == '__main__':
    ...
    try:
        ...
        if not os.path.exists(input_df_path):
            ...
            logging.info("Feature engineering complete. Output files saved:")
